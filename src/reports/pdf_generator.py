...
def generate_invoice_pdf(
    ...
    company_data = [
        [Paragraph(f"<b>{company.name}</b>", ParagraphStyle(
            "co_name", fontSize=16, alignment=TA_CENTER, spaceAfter=2
        ))],
        [Paragraph(
            f"GSTIN: {company.gstin or 'N/A'} | {company.address or ''}",
            style_center,
        )],
        [Paragraph(
            f"Phone: {company.phone or ''} | Email: {company.email or ''}",
            style_center,
        )],
    ]
    if company.bank_name:
        company_data.append([Paragraph(
            f"Bank: {company.bank_name}, {company.bank_branch} | A/c: {company.bank_account_no} | IFSC: {company.bank_ifsc}",
            style_center,
        )])

    tbl = Table(company_data, colWidths=[170*mm])
    ...
    elements.append(Paragraph(
        "<b>TAX INVOICE</b>",
        ParagraphStyle("inv_title", fontSize=14, alignment=TA_CENTER, spaceAfter=6),
    ))
    elements.append(Spacer(1, 2*mm))

    # Invoice info + Party details
    info_data = [
        [
    ...
    info_tbl = Table(info_data, colWidths=[100*mm, 70*mm])
    ...
    # Items table
    items_header = ["#", "Item (HSN)", "Qty", "Rate", "Taxable", "CGST", "SGST", "IGST", "Total"]
    ...
    items_data.append([
        ...
        f"{invoice.cgst_total:.2f}",
        f"{invoice.sgst_total:.2f}",
    ...
    # Amount in words
    elements.append(Paragraph(
        f"<b>Amount in Words:</b> {_num_to_words(invoice.grand_total)}",
        ParagraphStyle("words", fontSize=9, spaceAfter=6),
    ))

    # Tax summary
    tax_summary = Table([
        ["Tax Summary", "Rate", "Taxable", "CGST", "SGST", "IGST", "Total Tax"],
    ] + [
        [
            f"{item.gst_rate}%",
            f"{item.gst_rate}%",
    ...
    ]], colWidths=[30*mm, 20*mm, 25*mm, 20*mm, 20*mm, 20*mm, 25*mm])
    ...
    if company.bank_name:
        ...
        bank_tbl = Table(bank_data, colWidths=[170*mm])
    ...
    elements.append(terms_sig)
    elements.append(Spacer(1, 4*mm))

    # Footer
    elements.append(HRFlowable(width="100%", color=colors.grey))
    elements.append(Paragraph(
        f"Generated by TextileERP on {date.today().isoformat()}",
