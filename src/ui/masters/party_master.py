...
class PartyMasterWidget(QWidget):
    def __init__(self, session_factory, parent=None):
        ...
        layout.addWidget(make_header("Parties (Customers & Vendors)"))
    ...
    def _delete(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.information(self, "Select", "Please select a party to delete.")
            return
        party_id = self.table.item(row, 0).data(Qt.ItemDataRole.UserRole)
        ...
        session, svc = self._get_session_and_service()
        try:
            svc.delete_party(party_id)
            self._load()
        except Exception as e:
            QMessageBox.warning(self, "Cannot Delete", f"Party has existing transactions: {e}")
        finally:
            session.close()
