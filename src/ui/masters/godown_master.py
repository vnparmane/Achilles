...
class GodownMasterWidget(QWidget):
    ...
    def _delete(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.information(self, "Select", "Please select a godown to delete.")
            return
        godown_id = self.table.item(row, 0).data(Qt.ItemDataRole.UserRole)
        reply = QMessageBox.question(
            self, "Confirm Delete",
            "Delete this godown? This cannot be undone.",
        ...
        session, svc = self._get_session_and_service()
        try:
            svc.delete_godown(godown_id)
            self._load()
        except Exception as e:
            QMessageBox.warning(self, "Cannot Delete", f"Godown has existing transactions: {e}")
        finally:
            session.close()
